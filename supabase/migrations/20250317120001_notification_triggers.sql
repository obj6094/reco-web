-- Trigger: challenge_votes INSERT -> notify submission owner
...
CREATE TRIGGER tr_challenge_votes_insert
  AFTER INSERT ON challenge_votes
  FOR EACH ROW EXECUTE FUNCTION notify_challenge_vote();

-- Trigger: qna_ratings INSERT (Nice Reco) -> notify answer owner
...
CREATE TRIGGER tr_qna_ratings_insert
  AFTER INSERT ON qna_ratings
  FOR EACH ROW EXECUTE FUNCTION notify_nice_reco();

-- Trigger: qna_requests UPDATE best_answer_id -> notify answer owner
...
CREATE TRIGGER tr_qna_requests_best_answer
  AFTER UPDATE OF best_answer_id ON qna_requests
  FOR EACH ROW EXECUTE FUNCTION notify_answer_selected();

-- Trigger: qna_answers INSERT -> notify request owner
...
CREATE TRIGGER tr_qna_answers_insert
  AFTER INSERT ON qna_answers
  FOR EACH ROW EXECUTE FUNCTION notify_request_answered();
